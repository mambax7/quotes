...
function quotes_notify_iteminfo($category, $item_id)
{
    ...
    if ('category' === $category) {
        ...
        $sql           = 'SELECT _title FROM ' . $GLOBALS['xoopsDB']->prefix('quotes_cat') . ' WHERE _cid = ' . $item_id;
    ...
    if ('' == $category) {
        ...
        $sql           = 'SELECT _cid, _title FROM ' . $GLOBALS['xoopsDB']->prefix('quotes_category') . ' WHERE _lid = ' . $item_id;
        $result        = $GLOBALS['xoopsDB']->query($sql); // TODO: error check
        $resultArrayay = $GLOBALS['xoopsDB']->fetchArray($result);
        $item['name']  = $resultArrayay['title'];
        $item['url']   = XOOPS_URL . '/modules/' . $module->getVar('dirname') . '/quotes_visit.php?_cid=' . $resultArrayay['_cid'] . '&amp;_lid=' . $item_id;
